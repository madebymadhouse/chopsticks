        ...
        if (userPools && userPools.length > 0) {
          ...
          return await interaction.editReply({
            embeds: [
              buildInfoEmbed(
                "Choose a Pool",
                "Re-run this command with `pool:<pool_id>`.\nExample: `/agents add_token pool:pool_MadHouse`.",
                Colors.INFO
              ).addFields(
  ...
  if (sub === "list_tokens") {
    ...
    try {
      ...
      if (!requesterIsBotOwner) {
        ...
        tokens = allTokens.filter(token => accessiblePoolIds.has(token.pool_id || "pool_MadHouse"));
      }
      ...
      for (const token of tokens) {
        const poolId = token.pool_id || 'pool_MadHouse';
